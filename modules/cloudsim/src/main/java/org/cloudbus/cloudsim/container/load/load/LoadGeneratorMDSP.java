...
import org.cloudbus.cloudsim.container.load.UtilizationModelPlanetLabInMemoryExtended;
...
public class LoadGeneratorMDSP {
    private String propertiesPath = "/dev/xlx/CloudSimNFR-master/cloudsim-5.0/modules/cloudsim/src/main/java/org/cloudbus/cloudsim/container/load/load.properties";
    ...
    int containerCloudletNumbers = 0;
    ...
    public List<ContainerCloudlet> generateContainerCloudlets(Integer number) {
        ...
        for (Integer i = 0; i < number; i++) {
            ContainerCloudlet cloudlet =
                    new ContainerCloudlet(containerCloudletNumbers, length, pesNumber, fileSize,
                            outputSize, utilizationModel, utilizationModel,
                            utilizationModel);
            cloudlet.setUserId(brokeId);
            cloudlet.setVmId(vmId);
            containerCloudlets.add(cloudlet);
            containerCloudletNumbers += 1;
        }
    ...
    public static List<Load> createLoadList(int brokeId, List<String> Urls, UtilizationModel utilizationModelCpu, UtilizationModel utilizationModelRam, UtilizationModel utilizationModelBw) throws FileNotFoundException {
        ...
        long fileSize = 300L;
        long outputSize = 300L;
        String inputFolderName = org.cloudbus.cloudsim.container.load.LoadGeneratorMDSP.class.getClassLoader().getResource("workload/planetlab").getPath();
